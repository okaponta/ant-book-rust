...
fn main() {
    ...
    // sourceからお菓子
    for i in 0..n {
        mcf.add_edges(0, i + 1, 1, 0);
    }
    for i in n + 1..(m + 1) * n + 1 {
        mcf.add_edges(i, sink, 1, 0);
    }
    for i in 0..n {
        for j in 0..m {
            for k in 1..=n {
                let factory = (j + 1) * n + k;
                mcf.add_edges(i + 1, factory, 1, z[i][j] * k as i64);
            }
        }
    }
    let cost = mcf.min_cost_flow(0, sink, n as i64);
    println!("{:.*}", 2, cost as f64 / n as f64);
}
